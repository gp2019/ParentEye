...
class ArrayAdapterForShowMyFriends extends RecyclerView.Adapter {
    ...
    private Button remove,block;

    public ArrayAdapterForShowMyFriends(Context context, ArrayList<String> friends) {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder  {
        ...
        public ViewHolder(View itemView) {
            ...
            block  = itemView.findViewById(R.id.block_friend);

        }

    }

    private void remove(final String IdFreiends, final String idcurrentuser){
    ...
    private void block(final String IdFreiends, final String idcurrentuser){
        dbRef = FirebaseDatabase.getInstance().getReference().child( "Friends" );
            dbRef.child(idcurrentuser).child("blockFriends").addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    if (dataSnapshot.getValue(String.class) != null&&!dataSnapshot.getValue(String.class).isEmpty()) {
                        ...
                        dbRef.child(idcurrentuser).child("blockFriends").setValue(IdFreiends);
                    }
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder holder, final int position) {
        ...
        remove.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View view) {
                            remove(friendsArrayList.get(position),mAuth.getCurrentUser().getUid());
                        }
                    });
        ...
        block.setVisibility(View.VISIBLE);
                    block.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View view) {
                            remove(friendsArrayList.get(position),mAuth.getCurrentUser().getUid());
                            block(friendsArrayList.get(position),mAuth.getCurrentUser().getUid());
                        }
                    });
                ...
                if (users.getProfile_pic_id()!=null){
                    mStorageRef.child(users.getProfile_pic_id()).getBytes(ONE_MEGABYTE).addOnSuccessListener(new OnSuccessListener<byte[]>() {
                        ...
                        public void onSuccess(byte[] bytes) {
                            ...
                            ((ArrayAdapterForShowMyFriends.ViewHolder)holder).imageViewUser.setImageBitmap(bm);
                        }
                    });
                }

                ((ArrayAdapterForShowMyFriends.ViewHolder)holder).textViewUserName.setText(users.getUsername());
